...
export function createBabelTransformer({
  ...
  const formattedBabelTransformerPath = process.platform === 'win32' ? originalBabelTransformerPath.replace(/\\/g, '\\\\') : originalBabelTransformerPath;
  const babelTransformer = transformerTemplate
    .replaceAll('__BABEL_TRANSFORMER_PATH__', formattedBabelTransformerPath)
